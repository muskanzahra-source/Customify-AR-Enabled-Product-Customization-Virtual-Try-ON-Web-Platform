...
window.onload = function () {
  ...
  if (trackBtn) {
    trackBtn.addEventListener("click", trackMyOrder);
  }
...
document.addEventListener("DOMContentLoaded", function () {
  const images = ["images/shirt.jpg", "images/necklace copy.jpg", "images/Mugs.jpg", "images/pants.jpg"];
  let index = 0;
...
function openEmail() {
  ...
  window.open(`https://mail.google.com/mail/?view=cm&fs=1&to=${email}&su=${subject}&body=${body}`, '_parent');
}
// on click to contact us section whatapp
function openWhatsApp() {
  window.open("https://wa.me/923258253995", "_parent");
...
function openCustomizer(type) {
  ...
  const allCustomItems = [
    "necklace", "braslate", "keychain",
    "glasses", "printed", "frock", "festive", "suit"];

  if (allCustomItems.includes(type)) {
    setTimeout(() => {
      showCanvasGuideline();
    }, 600);
  }
...
async function removeBgFromStoredFile() {
  ...
  const oldNode = selectedNodes.find(node => node.getClassName() === 'Image' && node !== productImage);
...
function openCheckoutModal(cartItems = []) {
  ...
  document.getElementById("confirmBtn").onclick = async () => {
    ...
    if (!nameInput || !phoneInput || !addressInput || !emailInput) {
      alert("Please fill all required fields (Name, Phone, Address, Email)!");
      return;
    }
    ...
    if (typeof stage !== 'undefined' && stage) {
      if (typeof tr !== 'undefined') tr.nodes([]);
      layer.draw();
      customDesignImage = stage.toDataURL({ pixelRatio: 2 });
    }

    const normalImagesList = cartItems.map(item => item.image).join(", ");
    ...
    try {
      ...
      if (text.includes("Order placed successfully")) {
        ...
        saveOrderToLocal(subtotal, [...cartItems]);
        modal.remove();
        cartItems.length = 0;
        if (typeof updateCartBadge === "function") updateCartBadge();
        if (typeof updateTrackBadge === "function") updateTrackBadge();
        saveOrderToLocal(subtotal);
      } else {
...
function addfeedback() {
  ...
  const senderName = "Customify";
  const phpUrl = `./Herosection.php?feedback=${encodeURIComponent(text)}&send=${encodeURIComponent(senderName)}`;
  fetch(phpUrl)
    ...
    .catch(error => {
      ...
      alert("Connection failed! Check if XAMPP MySQL is running.");
    });
}
...
function onPoseResults(results) {
  ...
  if (currentProductType === "glasses") {
    ...
    targetScale = (eyeDist / productImage.width()) * 3.4 * sizeMultiplier;
  ...
  //Event Sash
    ...
    targetScale = (sashLength / productImage.height()) * 1.1 * sizeMultiplier;
  ...
  //braslate fitting on wrist
    ...
    targetX = newX;
    targetY = newY;
    targetScale = (W * 0.15 / productImage.width()) * sizeMultiplier;
    targetAngle = 0;
    offX = productImage.width() / 2;
  ...
  // Pant fitting
    ...
    targetScale = (hipWidth / productImage.width()) * 2.8 * sizeMultiplier;
  ...
  // School bag fitted on shoulders and back
    ...
    const shoulderDist = Math.hypot(RSx - LSx, RSy - LSy);
    targetScale = (shoulderDist / productImage.width()) * 1.65 * sizeMultiplier;
    targetAngle = Math.atan2(RSy - LSy, RSx - LSx) * 180 / Math.PI;
    offX = productImage.width() / 2;
  ...
  //Necklace setting on neck
    ...
    const shoulderDist = Math.hypot(RSx - LSx, RSy - LSy);
    targetScale = (shoulderDist / productImage.width()) * 0.8 * sizeMultiplier;
    targetAngle = Math.atan2(RSy - LSy, RSx - LSx) * 180 / Math.PI;
    offX = productImage.width() / 2;
  ...
  //MUG fitting 
    ...
    targetScale = (handDist / productImage.width()) * 3.8 * sizeMultiplier;
...
//displaying guideline msg on canvas
function showCanvasGuideline() {
  const customItems = ["necklace", "braslate", "keychain", "glasses", "printed", "frock", "festive", "suit"];
  if (!customItems.includes(currentProductType)) return;

  const oldGuide = layer.findOne('#guideGroup');
  if (oldGuide) oldGuide.destroy();

  const guideGroup = new Konva.Group({ id: 'guideGroup' });

  const rectWidth = 460;
  let rectHeight = 65;

  if (["glasses", "printed", "frock", "festive", "suit"].includes(currentProductType)) {
    rectHeight = 45;
  }
  ...
  let message = "";
  if (["necklace", "braslate", "keychain"].includes(currentProductType)) {
    message = "Add your name to personalize the product, or if you do not prefer the current design, upload your own design on the canvas and order a fully customized product.";
  } else {
    message = "If you do not prefer the current design, upload your own design on the canvas and order a fully customized product.";
  }

  const guideText = new Konva.Text({
    x: stage.width() / 2 - (rectWidth / 2) + 15,
    y: 15,
    text: message,
    width: rectWidth - 30,
    height: rectHeight,
